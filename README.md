...
1. Each module includes a built-in Jupyter notebook designed to summarize data efficiently and visually. The following visualization generated by Dataviz.ipynb using the included test dataset. The test dataset is a toy human gut microbiome i.e.: simulated Illumina short reads covering 4 bacterial reference genomes from the Unified Human Gut Genome at 10X coverage per genome:
    ...
    To compare MAGs from different samples using the field-standard quality metrics described above (see example below), follow the instructions in the Jupyter notebook. For most panels, each sample (set of MAGs) is represented by a different colour:
```Bash
jupyter notebook &
```
<img width="1965" height="1896" alt="mag_qc_0" src="https://github.com/user-attachments/assets/cbf6812e-145d-4bfd-83de-b6700553f14a" />
